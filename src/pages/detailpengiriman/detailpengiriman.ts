...
export class DetailpengirimanPage {
  ...
  doCheckBarang(kirim) {
    this.api.get("table/delivery_order_line", { params: { limit: 100, filter: "receipt_no='" + kirim.receipt_no + "'", sort: "part_no" + " ASC " } })
      .subscribe(val => {
        this.itemsall = val['data']
        this.checkshow = true;
      }, err => {
        this.doCheckBarang(kirim)
      });
  }
  doOffAdd() {
    this.datapart = [];
  ...
  doConfirmationLoading(part) {
    ...
    this.api.put("table/delivery_order_line",
      ...
      .subscribe(
        (val) => {
          this.doGetdolCheckLoading(part)
        });
  }
  doGetdolCheckLoading(part) {
    this.api.get("table/delivery_order_line", { params: { limit: 100, filter: "receipt_no='" + part.receipt_no + "' AND check_loading=0", sort: "part_no" + " ASC " } })
      .subscribe(val => {
        let data = val['data']
        if (data.length == 0) {
          this.doGetSlotDelivery(part)
        }
        else {
          this.doGetdol(part)
        }
  ...
  doGetSlotDelivery(part) {
    this.api.get("table/slot_delivery", { params: { limit: 100, filter: "receipt_no='" + part.receipt_no + "'" } })
      .subscribe(val => {
        let data = val['data']
        this.doUpdateSlotDelivery(part, data)
      });
  }
  doUpdateSlotDelivery(part, data) {
    const headers = new HttpHeaders()
      .set("Content-Type", "application/json");
  ...
  doGetdol(part) {
    this.api.get("table/delivery_order_line", { params: { limit: 100, filter: "receipt_no='" + part.receipt_no + "'", sort: "part_no" + " ASC " } })
      .subscribe(val => {
        this.itemsall = val['data']
        this.doGetKirimNew()
      });
  ...
  doGetKirimNew() {
    this.api.get("table/slot_delivery", { params: { limit: 100, filter: "plat_no='" + this.navParams.get('data')[0].plat_no + "' AND date_delivery=" + "'" + moment().format('YYYY-MM-DD') + "'", sort: 'no_urut_group ASC' } })
      .subscribe(val => {
