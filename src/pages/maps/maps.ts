...
export class MapsPage {
  ...
  constructor(
    ...
    this.backgroundMode.on("activate").subscribe(() => {
      this.api.get("table/slot_delivery", { params: { limit: 100, filter: "plat_no='" + this.navParam.get('kirim')['plat_no'] + "' AND date_delivery=" + "'" + moment().format('YYYY-MM-DD') + "' AND uuid=" + "'" + this.navParam.get('kirim')['uuid'] + "'", sort: 'no_urut_group ASC' } })
        .subscribe(val => {
  ...
  doUpdateSlotDeliveryShipment() {
    ...
    this.api.put("table/slot_delivery",
      ...
      .subscribe(
        (val) => {
          this.api.get("table/slot_delivery", { params: { limit: 100, filter: "plat_no='" + this.kirim['plat_no'] + "' AND date_delivery=" + "'" + moment().format('YYYY-MM-DD') + "'", sort: 'no_urut_group ASC' } })
            .subscribe(val => {
